import classes from '../../AllPrograms.module.css'
import legs1 from '../../../../../public/Programs_thumbnail/legs/legs_beginner.png'
import legs2 from '../../../../../public/Programs_thumbnail/legs/legs_intermediate.png'
import legs3 from '../../../../../public/Programs_thumbnail/legs/legs_advance.png'

function Legs() {
  return (
    <div className="flex gap-2 pb-10 pt-3">
      {/*  */}
      <div className={`${classes.card} bg-white w-1/4 mx-auto mt-5 pb-4 rounded-3xl shadow-md`}>
        <img src={legs1} className={`mx-auto rounded-t-3xl`} />
        <div className={`${classes.Tags} flex gap-3 mt-5 mb-3 px-5`}>
          <div className="rounded-3xl border-black border px-2 bg-[#f3f3f3]">Beginner</div>
          <div className="rounded-3xl border-black border px-2 bg-[#f3f3f3]">Legs</div>
        </div>
        <div className={`${classes.CardTitle} px-5`}>Legs workout for beginner</div>
      </div>
      {/*  */}

      {/*  */}
      <div className={`${classes.card} bg-white w-1/4 mx-auto mt-5 pb-4 rounded-3xl shadow-md`}>
        <img src={legs2} className={`mx-auto rounded-t-3xl`} />
        <div className={`${classes.Tags} flex gap-3 mt-5 mb-3 px-5`}>
          <div className="rounded-3xl border-black border px-2 bg-[#f3f3f3]">Intermediate</div>
          <div className="rounded-3xl border-black border px-2 bg-[#f3f3f3]">Legs</div>
        </div>
        <div className={`${classes.CardTitle} px-5`}>Intermediate Leg Dynamo</div>
      </div>
      {/*  */}
      {/*  */}
      <div className={`${classes.card} bg-white w-1/4 mx-auto mt-5 pb-4 rounded-3xl shadow-md`}>
        <img src={legs3} className={`mx-auto rounded-t-3xl`} />
        <div className={`${classes.Tags} flex gap-3 mt-5 mb-3 px-5`}>
          <div className="rounded-3xl border-black border px-2 bg-[#f3f3f3]">Advance</div>
          <div className="rounded-3xl border-black border px-2 bg-[#f3f3f3]">Legs</div>
        </div>
        <div className={`${classes.CardTitle} px-5`}>Legendary legs workout</div>
      </div>
      {/*  */}
    </div>
  )
}

export default Legs
